...
def initialize_model(model_name: str, dummy_input: torch.Tensor, rank: int, device: torch.device):
    ...
    if model_name.lower() == "borzoi" or model_name.lower() == "enformer" :
        ...
        if rank == "full":
            ...
            quant_path = os.path.join(lora_weights_dir, f"{base_name}_quant.pth")

            # --- FX Quantization ---
            print("[INFO] FX preparing model for quantization...")
            _ = model(dummy_input)  # dummy forward
            qconfig = get_default_qconfig("fbgemm")
            qconfig_mapping = QConfigMapping().set_global(qconfig)
            prepared_model = quantize_fx.prepare_fx(model, qconfig_mapping, dummy_input)
            print("[INFO] Converting FX graph to quantized model...")
            model_quantized = quantize_fx.convert_fx(prepared_model)

        # Load pre-quantized weights
            if os.path.exists(quant_path):
                print(f"[INFO] Loading pre-quantized weights: {quant_path}")
                state = torch.load(quant_path, map_location="cpu")
                model_quantized.load_state_dict(state)
                is_quantized = True
            else:
                print("[WARN] Quantized weights not found, using FX quantized model with FP weights")
...
def main(
    ...
    for rank in ranks:
        ...
        csv_name = f"{model_name}_count_times.csv"
