...
def calculate_correlation(model_name):
    ...
    full_model = initialize_models(k_l = None, quant = False, model_name = model_name, full = True)
    full_model.to(device)
    pearson_corrs_ranks = {}

    for r in ranks:
        pearson_corrs = []
        lora_model = initialize_models(k_l = r, quant = False, model_name = model_name, full = False)
        ...
        for batch in progress_bar:
            ...
            data = data.to(device)
            with torch.no_grad():
                out_full = full_model(data)
                out_lora = lora_model(data)
...
def main():
    borzoi_corrs = calculate_correlation("borzoi")
    # Save results
    with open("borzoi_pearson_corrs_shuffle.pkl", "wb") as f:
        pickle.dump(borzoi_corrs, f)
    enformer_corrs = calculate_correlation("enformer")
    with open("enformer_pearson_corrs_shuffle.pkl", "wb") as f:
        pickle.dump(enformer_corrs, f)
    sei_corrs = calculate_correlation("sei")
    with open("sei_pearson_corrs_shuffle.pkl", "wb") as f:
