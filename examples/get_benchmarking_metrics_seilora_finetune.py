...
def initialize_models(rank: int, trained_version: str, quant: bool, full = False, file_path = None):
    ...
    if file_path == None:
        if not full:
            # cp_model_seq = sl.Sei_LLRA(k=rank, ft = trained_version, projection = False, mode = "sequence", quant = q)
            # cp_model_var = sl.Sei_LLRA(k=rank, ft = trained_version, projection = False, mode = "variant", quant = q)
            # sc_model_seq = sl.Sei_LLRA(k=rank, ft = trained_version, projection = True, mode = "sequence", quant = q)
            sc_model_var = sl.Sei_LLRA(k=rank, ft = trained_version, projection = True, mode = "variant", quant = q)
        ...
        if trained_version == "probe":
            state_dict = torch.load(file_path)
            projection = seiSmallPhead()
            projection.load_state_dict(state_dict, strict = True)
            return projection
        elif trained_version == "ft_proj":
            state_dict = torch.load(file_path)
            ft_model = sm.get_sei_projection()
            ft_model.set_mode("variant")
            ft_model.load_state_dict(state_dict, strict = True)
            return ft_model
        elif trained_version == "ft_head":
            state_dict = torch.load(file_path)
            ft_model = HeadMod(rank)
            ft_model.load_state_dict(state_dict, strict = True)
            return ft_model
...
def get_gtex_eqtls_promoter(model, rank, trained_version = ""):
    ...
    outs = get_over_under_null(sc_ref, sc_alt, vcf, df, trained_version)
    return outs 
...
def get_eu_lcl_caqtls(model, rank, trained_version = ""):
    ...
    df_combine.to_csv(f"scores/caqtls_eu_lcl_seilora_rank{rank}.tsv.gz", sep="\t", index=False, compression="gzip")
...
def get_yoruba_lcl_dsqtls(model, rank, trained_version = ""):
    ...
    ap_unsigned = average_precision_score(df_combine["obs.label"], abs(df_combine["GM12878_DNase_cp_mean"]))
    df_combine.to_csv(f"scores/dsqtls_yoruba_lcl_seilora_rank{rank}.tsv.gz", sep="\t", index=False, compression="gzip")
...
def get_afr_lcl_caqtls(model, rank, trained_version = ""):
    ...
    df_combine = df.merge(df_pred, left_on = ["var.chr", "var.pos_hg38"], right_on=["CHROM", "POS"], how = "inner")
    ap_unsigned = average_precision_score(df_combine["obs.label"], abs(df_combine["GM12878_DNase_cp_mean"]))
    df_combine.to_csv(f"scores/caqtls_afr_lcl_seilora_rank{rank}.tsv.gz", sep="\t", index=False, compression="gzip")
...
def get_microglia_caqtls(model, rank, trained_version = ""):
    ...
    df_pred = pd.DataFrame(vcf, columns=["CHROM", "POS", "NAME", "REF", "ALT"])
    df_pred["POS"] = df_pred["POS"].astype(int)
    df_pred["Macrophage_ATAC_cp_mean"] = get_celltype_asssy_specific(diff, celltypes = ["Macrophage"], assays = ["ATAC-seq"], strict = True)

    df_combine = df.merge(df_pred, left_on = ["var.chr", "var.pos_hg38"], right_on=["CHROM", "POS"], how = "inner")
    df_combine.to_csv(f"scores/caqtls_eu_microglia_seilora_rank{rank}.tsv.gz", sep="\t", index=False, compression="gzip")
...
def get_smc_caqtls(model, rank, trained_version = ""):
    ...
    df_combine = df.merge(df_pred, left_on = ["var.chr", "var.pos_hg38"], right_on=["CHROM", "POS"], how = "inner")
    df_combine.to_csv(f"scores/caqtls_eu_smc_seilora_rank{rank}.tsv.gz", sep="\t", index=False, compression="gzip")
...
def get_spi1_bqtls(model, rank, trained_version = ""):
    ...
    df_combine.to_csv(f"scores/bsqtls_eu_spi1_seilora_rank{rank}.tsv.gz", sep="\t", index=False, compression="gzip")
...
def get_variants(model, vcf, rank, benchmark_name="", trained_version = "", sc = False):
    ...
    if trained_version == "probe" or trained_version == "ft_proj":
        ...
        emb_model = TrunkScoreMod(rank)
        emb_model.eval()
        dataloader =  EmbeddingDataLoader(dataset=dataset, model = emb_model, batch_size=32, shuffle=False, num_workers=7, device = "cpu")
    else:
        dataloader = VariantDataLoader(dataset=dataset, batch_size=32, shuffle=False, num_workers=7)
    device = "cpu"
    ...
    for batch in progress_bar:
        ...
        if trained_version == "probe":
            all_cp_ref.append(cp_outputs.detach().cpu())
        else:
    ...
    if trained_version == "probe":
        return np.zeros_like(all_cp_ref), all_cp_ref, all_vcf
    else:
...
def save_output(rank = 256, trained_version = None, quant = False, full = False, file_path = None):
    if quant:
        q = "quant"
    else:
        q = "no_quant"
    ...
    # bpn_path = "benchmark_chrombpnet_all.tsv"
    # bpn_row_dict = {
    #     "model": model_name,
...
def main():
    ...
    save_output(rank=256, trained_version = "ft_head", quant = True, file_path="./checkpoint_256/best_model_pai_finetune_head_v1.pt")
    save_output(rank=1, trained_version = "ft_head", quant = True, file_path="./checkpoint_1/best_model_pai_finetune_head_v1.pt")
    
    save_output(rank=256, trained_version = "probe", quant = True, file_path="./checkpoint_256/best_model_pai_probe_v1.pt")
    save_output(rank=1, trained_version = "probe", quant = True, file_path="./checkpoint_1/best_model_pai_probe_v1.pt")
